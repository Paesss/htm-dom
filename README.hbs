...
With `{{package.name}}`, the static template and dynamic values remain separate:
...
`innerHTML` accepts a string. `{{package.name}}` accepts DOM nodes, nested arrays, strings, numbers, and components in the same child position:
...
An `innerHTML` string can describe attributes, but it cannot assign live object properties or attach event listeners without a second pass through the DOM. `{{package.name}}` does both during creation:
...
`{{package.name}}` creates the final DOM nodes directly. There is no virtual DOM, JSX compiler, rendering runtime, or string-to-DOM round trip to understand:
...
Choose `{{package.name}}` when dynamic values, direct DOM behavior, small components, and TypeScript types matter more than serializing a string.
...
This source uses Handlebars' Mustache-style templates. Package metadata is available under `package`, so `{{package.name}}` and `{{package.version}}` are replaced from `package.json` during the docs build.
