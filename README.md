...
This source uses Handlebars' Mustache-style templates. Package metadata is available under `package`, so `htm-dom` and `0.1.2` are replaced from `package.json` during the docs build.
